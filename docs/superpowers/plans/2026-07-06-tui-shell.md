...
all** — it calls `tools.get(&call.name)` and `tool.execute_erased(...)` directly, bypassing the
`PermissionMiddleware` Phase 2 built for the non-streaming `Agent::prompt` path entirely. Since the
TUI must stream (spec section 5) and must still gate bash/write/edit calls, this plan does **not**
reuse Phase 2's `PermissionMiddleware`/`build_agent` for the interactive session. Instead it adds a
`GatedTool<T>` wrapper (`src/tui/gated_tool.rs`) that embeds the permission check *inside each
tool's own `execute`*, which both `prompt` and `prompt_stream` call unconditionally — so gating
works correctly under streaming. `PermissionGate`, `PermissionPrompter`, `PermissionRequest`,
`PermissionDecision`, and `PermissionTier` (Phase 2's exact types, `local_code::permissions::*`)
are reused completely unchanged; only the enforcement *plumbing* differs from headless mode.
...
- `local_code::tui::gated_tool::{GatedTool, build_streaming_agent}` — the streaming-safe permission
  enforcement Phase 4's `/permissions` tier switch and `/model` connection switch must rebuild
  through, not around.
...
`local_code::agent::provider::build_model`, `local_code::agent::tools::{ReadFile, WriteFile,
...
- Create: `src/tui/gated_tool.rs` — `GatedTool<T>`, `build_streaming_agent`
...
```rust
//! Streaming-safe permission-gated tool wrapper, filled in by Task 3.
```
...
### Task 3: `GatedTool` — streaming-safe permission enforcement
...
/// [`PermissionGate`] before doing real work, rather than relying on
/// `daimon::middleware::Middleware` (which `Agent::prompt_stream` — confirmed
/// by reading `daimon-0.16.0/src/agent/runner.rs` — never invokes; it calls
/// `tool.execute_erased(...)` directly while accumulating streamed tool-call
/// deltas). Embedding the check inside `execute` itself makes permission
/// enforcement work identically whether the agent is driven via `prompt` or
/// `prompt_stream`.
...
/// Builds the `daimon::agent::Agent` used by the interactive TUI: the same six
/// built-in tools as headless mode's `local_code::agent::build::build_agent`,
/// each wrapped in [`GatedTool`] instead of relying on
/// `local_code::agent::middleware::PermissionMiddleware`. Read-only tools are
/// wrapped too (harmless — [`PermissionGate::check`] always allows
/// `ToolKind::ReadOnly` without prompting) so every tool goes through one
/// uniform path.
...
Run: `cargo test --lib tui::gated_tool`
Expected: PASS (6 tests). This is the load-bearing proof that permission enforcement survives
under `prompt_stream` — if `built_streaming_agent_streams_a_response` passes and
`denied_edit_tool_never_touches_the_filesystem` passes, the streaming-gating design is verified,
...
git commit -m "feat: gate built-in tools at execute() so prompt_stream enforces permissions too"
```
...
- **Type consistency:** `TranscriptEntry`/`ToolCallEntry`/`ToolCallResult`/`UsageSummary` (Task 2)
  ...
  `GatedTool`/`build_streaming_agent` (Task 3) are used only by `App` (Task 7) — headless mode's
  `build_agent`/`PermissionMiddleware` from Phase 2 are untouched and still used only by
  `run_headless`. `NtuiPermissionPrompter` (Task 4) implements `local_code::permissions::types::
...
- **API-compatibility risks worth flagging before implementation starts:**
  1. **`Agent::prompt_stream` bypasses `daimon::middleware::Middleware` entirely** — verified by
     ...
     `self.middleware.run_on_tool_call(&mut call_mut).await` at line ~318). This is the single
     biggest risk this plan works around, via `GatedTool` (Task 3) checking the gate inside each
     tool's own `execute` instead of relying on middleware. If a future `daimon` upgrade adds
     middleware support to `prompt_stream`, `GatedTool` and Phase 2's `PermissionMiddleware` would
     then both be valid options — no correctness bug, just redundant enforcement paths worth
